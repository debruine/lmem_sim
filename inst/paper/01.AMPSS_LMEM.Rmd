...
tibble::tribble(~`model`, ~`code`, ~`description`,
        "$RT_{si}$", "`RT`", "reaction time for subject $s$ to item $i$",
        "$X_i$", "`X_i`", "condition for item $i$ (-.5 = ingroup, .5 = outgroup)",
        "$\\beta_0$", "`b0`", "intercept; grand mean RT",
        "$\\beta_1$", "`b1`", "slope; mean effect of ingroup/outgroup",
        ...
        "$\\omega_0$", "`omega_0`", "standard deviation of by-item random intercepts",
  ...
  apa_table(align = c("l", "l", "l"),
            caption = "Parameters used in the data-generating model and associated R code.",
            escape = TRUE,
            placement = "H")
...
Now you can generate a dataset with the default parameters using `my_sim_data()` or, for example, a dataset with 500 subjects and no effect of category using `my_sim_data(n_subj = 500, b1 = 0)`.
...
* the error term (`e_si`) is automatically included in all models, so is left implicit
