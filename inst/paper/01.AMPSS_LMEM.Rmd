...
Finally, we need to characterize the trial-level noise in the study ($e_{si}$) in terms of its standard deviation. Here we simply assign this parameter value `sigma` to be twice the size of the by-subject random intercept SD. Later, we will 
...
n_ingroup <- 25L # 25 ingroup stimuli
n_outgroup <- 25L # 25 outgroup stimuli
```
...
trials <- crossing(subjects, items)  %>%
  mutate(e_si = rnorm(n_subj * (n_ingroup + n_outgroup), # N subj x N stimuli
                     mean = 0, sd = sigma)) %>%
...
my_lmer_power(n_ingroup = 50, n_outgroup = 50, b1 = 20)
```
