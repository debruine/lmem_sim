...
```{r params}
nsubj  <- 100 # number of subjects pre between-subject cell
nitem  <- 25  # number of items per between-item cell
```

Then we specify the fixed effects. The overall mean (`mu`) is 800 ms and the difference between conditions A and B (`eff`) is 80 ms. 

**EXPLAIN EFFECT CODING**
...
Now we specify the variance for the random effects by items, subjects, and error.
...
Condition is a between-items factor, so we need to include it in the `between` argument. Set `n = nitem` to specify `r nitem` items per condition. There are no within-items factors, so we only need to simulate one random effect: the  intercept. Set `sd = iri_sd` to set the standard deviation for this random effect (the mean deafults to 0). Set `dv = "iri"` to give the DV column that name. Set `id = "item_id"`; we'll use this later to join this information to the table of trials.

** EFFECT CODE CONDITION **

```{r sim-items, out.width = "75%", fig.cap="The distribution of random effects for items in conditions A and B"}
...
Now that we have a table of all trials, we can join the information from our `subjects` and `items` tables using the `subj_id` and `item_id`. Calculate the reponse (`Y`) as the sum of the grand intercept (`mu`), the subject-specific random intercept (`sri`), the item-specific random intercept (`iri`), the effect of condition (`eff`) plus the subject-specific random slope of condition (`srs`) multiplied by the effect-coded condition (`cond`) plus the random error (`err`).
...
Now we're ready to analyse our simulated data. Set up the formula for `lmer()` as the response, predicted by the factors, plus random effects for items and subjects. Items only have a random intercept, which is represented by the `1` in `(1 | item_id)`. Subjects have a random intercept and a random slope for condition, so the random effects are specified as `(1 + cond | subj_id)`.
